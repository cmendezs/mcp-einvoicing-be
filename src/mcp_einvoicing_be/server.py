...
def _register_be_tools(server: EInvoicingMCPServer) -> None:
    """Register all Belgian e-invoicing tools onto the shared server."""
    server.tool()(_validator.validate_invoice_be)
    server.tool()(_validator.validate_pint_be)
    server.tool()(_generator.generate_invoice_be)
    server.tool()(transform_to_ubl)
    server.tool()(lookup_vat_be)
    server.tool()(check_peppol_participant_be)
    server.tool()(get_invoice_types_be)
...
mcp.register_plugin(_register_be_tools)
